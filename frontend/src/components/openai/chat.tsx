      ...
      <div className={`flex items-start gap-2.5 ${role === "user" ? "flex-row-reverse" : "flex-row"}`}>
        ...
        <div
          className={`flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 ${
            role === "user" ? "bg-muted text-primary-foreground rounded-s-xl" : " bg-primary rounded-se-xl "
          } rounded-b-xl`}
        >
          {role === "assistant" ? (
...
export default function ChatComponent() {
  ...
  const [chatLog, setChatLog] = useState<{ role: "user" | "assistant"; content: string }[]>([
    { role: "assistant", content: "Hola! Soy el asistente virtual de GPI360. ¿En qué te puedo ayudar?" },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      setLoading(true);
      const response = await postChatMessageService({ conversation: chatLog, message });
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mb-60">
      ...
      <CardFooter>
        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); setMessage(""); }} className="flex w-full gap-2">
          <Input
            type="text"
            value={message}
