    ...
    putProduct(id, data) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const { stock } = data, productData = __rest(data, ["stock"]);
                if (productData.subCategoryId === "") {
                    delete productData.subCategoryId;
                }
                yield existingProduct.update(productData);
                if (stock) {
                    const stockUpdates = stock.map((item) => StockServices_1.default.putStock(item.id, item));
